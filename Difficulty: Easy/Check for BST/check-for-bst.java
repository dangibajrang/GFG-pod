...
class Solution {
    ...
    boolean isBST(Node root, Integer minValue, Integer maxValue) {
        if (root == null) {
            return true;
        }
        
        // The value of the current node must be within the range [minValue, maxValue]
        if ((minValue != null && root.data <= minValue) || 
            (maxValue != null && root.data >= maxValue)) {
            return false;
        }
        
        // Recursively check the left subtree with updated maxValue
        // and the right subtree with updated minValue
